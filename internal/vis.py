...
def visualize_cmap(value,
  ...
  if colormap:
    ...
    colorized = value
  if white_background:
    colorized = colorized + (1. - weight[Ellipsis, None])
...
def visualize_suite(rendering, rays, config):
  ...
  vis_mean, vis_median = [
      visualize_cmap(x, acc, cm.get_cmap('turbo'), curve_fn=depth_curve_fn, matte_background=False, white_background=config.white_background)
      for x in [distance_mean, distance_median]
  ]

  vis_mean_mod, vis_median_mod = [
      visualize_cmap(x, acc, sinebow, modulus=0.1, curve_fn=depth_curve_fn, matte_background=False, white_background=config.white_background)
      for x in [distance_mean, distance_median]
  ]
...
def visualize_weight(weight, z_vals, filename, color):
  plt.figure(figsize=(4, 2))
  plt.plot(z_vals, weight, color=color)
